...
class SettingAccessForm(forms.Form):
...
class PersonalGoalForm(forms.ModelForm):
    class Meta:
        model = PersonalGoal
        fields = ["shared_note", "philosophy_goal", "finance_goal", "safety_goal", "cooperation_goal"]
        labels = {
            "shared_note": "自由記載",
        ...
        widgets = {
            "shared_note": forms.Textarea(
                attrs={
                    "rows": 5,
                    "maxlength": 400,
                    "placeholder": "補足事項や共有したい内容を入力してください。",
                }
            ),
...
class EvaluationForm(forms.Form):
    ...
    philosophy_eval = forms.CharField(
        label="理念AI評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "data-ai-field": "philosophy"}),
    )
    finance_eval = forms.CharField(
        label="経営AI評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "data-ai-field": "finance"}),
    )
    safety_eval = forms.CharField(
        label="安全AI評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "data-ai-field": "safety"}),
    )
    cooperation_eval = forms.CharField(
        label="連携AI評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4, "data-ai-field": "cooperation"}),
    )
    philosophy_manager_eval = forms.CharField(
        label="理念上長評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4}),
    )
    finance_manager_eval = forms.CharField(
        label="経営上長評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4}),
    )
    safety_manager_eval = forms.CharField(
        label="安全上長評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4}),
    )
    cooperation_manager_eval = forms.CharField(
        label="連携上長評価",
        required=False,
        widget=forms.Textarea(attrs={"rows": 4}),
    )
